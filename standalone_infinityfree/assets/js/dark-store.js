...
(function() {
    ...
    function initAjaxAddToCart() {
        ...
        function checkGuestPurchase(e) {
            if (window.cms_store_vars && !window.cms_store_vars.is_logged_in) {
                if (e) {
                    e.preventDefault();
                    e.stopPropagation();
                }
                showCartErrorToast('⚠️ Vui lòng đăng nhập hoặc tạo tài khoản để đặt mua hàng!');
                setTimeout(function() {
                    window.location.href = window.cms_store_vars.login_url || '/my-account/';
                }, 850);
                return true;
            }
            return false;
